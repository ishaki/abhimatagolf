...
def test_leaderboard_api():
    ...
    try:
        response = requests.get(f"{base_url}/api/v1/leaderboards/event/3?use_cache=false", headers=headers)
        print(f"   Status: {response.status_code}")
    ...
    try:
        response = requests.get(f"{base_url}/api/v1/leaderboards/public/event/3")
        print(f"   Status: {response.status_code}")
