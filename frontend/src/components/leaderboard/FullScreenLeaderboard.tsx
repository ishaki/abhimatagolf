...
import { RefreshCw, Trophy, Filter, Minimize2, Play, Pause, RotateCcw, Clock } from 'lucide-react';
...
interface FullScreenLeaderboardProps {
  ...
  onClose: () => void;
  onScoreUpdate?: () => void;
  autoScrollInterval?: number; // Auto-scroll interval in seconds (default: 9)
  recordsPerPage?: number; // Records to show per page (default: 10)
...
const FullScreenLeaderboard: React.FC<FullScreenLeaderboardProps> = ({
    ...
    const startAutoScroll = () => {
      scrollIntervalRef.current = setInterval(() => {
        setCurrentScrollIndex(prev => {
          ...
          const nextIndex = prev + recordsPerPage;
          // If we've reached or passed the end, loop back to start
          if (nextIndex >= leaderboard.entries.length) {
            return 0;
          }
          return nextIndex;
        });
  ...
  }, [leaderboard?.entries.length, isAutoScrolling, isHovered, autoScrollInterval, recordsPerPage]);
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col">
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mt-1 text-sm text-gray-600">
              ...
              <span>
                {leaderboard.participants_with_scores} With Scores
              </span>
              <span>
                Course Par: {leaderboard.course_par}
              </span>
              <span>
                Last Updated: {formatLastUpdated(leaderboard.last_updated)}
              </span>
            </div>
          </div>
          ...
          {leaderboard?.entries.length > 0 && isAutoScrolling && (
            <div className="flex items-center space-x-2 text-sm text-gray-600">
              ...
              <span>
                Page {Math.floor(currentScrollIndex / recordsPerPage) + 1} / {Math.ceil(leaderboard.entries.length / recordsPerPage)}
              </span>
              <div className="w-16 bg-gray-200 rounded-full h-2">
                <div
                  className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                  style={{
                    width: `${(Math.floor(currentScrollIndex / recordsPerPage) + 1) / Math.ceil(leaderboard.entries.length / recordsPerPage) * 100}%`
                  }}
                />
              </div>
            </div>
      ...
      {/* Filters */}
      {showFilters && (
        <div className="bg-gray-50 border-b border-gray-200 px-8 py-4 flex-shrink-0">
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {leaderboard.entries.map((entry: LeaderboardEntry) => (
                      <tr key={entry.participant_id} className="border-b border-gray-100 hover:bg-gray-50">
